...
link = 'https://sf.jumpbikes.com/opendata/free_bike_status.json'  ## enter bikeshare data link here

# get the bikeshare data from the internet and write to a blank file in csv format

bike_data = single_query(link)
with open('sf-bike-data.csv', mode='w+') as f:
...
#for k in [1,2,3,4]:
    ...
    bike_data = single_query(link)
    with open('sf-bike-data.csv', mode='a') as f:
